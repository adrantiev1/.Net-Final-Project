...
{   
    ...
    public class SecurityController
    {
        ...
        private void CheckResult(IdentityResult result)
        {
            if (!result.Succeeded)
                throw new Exception($@"Security changes were not applied:<ul>
                                        {string.Join(string.Empty, result.Errors.Select(x => $"<li>{x}</li>"))}</ul>");
        }
        #endregion

        #region ApplicationUser CRUD
        [DataObjectMethod(DataObjectMethodType.Select)]
        public List<RegisteredUser> ListUser()
        {
            //ApplicationsUser objects into collection of RegisteredUser objects
        ...
        public void UpdateUser(RegisteredUser user)
        {
            // NOTE: Ideally, we should be doing this in a TransactionScope to ensure
            //       that it all succeeds or fails as a group.
            var existing = UserManager.FindById(user.UserId.ToString());
            if (existing == null)
            ...
            if (removeRoles.Any())
            {
                result = UserManager.RemoveFromRoles(user.UserId.ToString(),
                                                     removeRoles.ToArray());
                CheckResult(result);
            }
            ...
            if (newRoles.Any())
            {
                result = UserManager.AddToRoles(user.UserId.ToString(),
                                                newRoles.ToArray());
                CheckResult(result);
            }
